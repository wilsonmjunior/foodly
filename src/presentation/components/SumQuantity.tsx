...
export function SumQuantity({ quantity, onDecrement, onIncrement }: SumQuantityProps) {
    return (
        <div className="flex items-center gap-3">
            <button
                className={`${quantity !== 1 ? 'w-8 h-8 rounded-full border-2 border-teal-400' : ''} flex items-center justify-center text-lg text-teal-400`}
                onClick={onDecrement}
                type="button"
            >
                {quantity === 1 ? (
            ...
            <button
                className="w-8 h-8 rounded-full border-2 border-teal-400 flex items-center justify-center text-lg text-teal-400"
                onClick={onIncrement}
                type="button"
            >
